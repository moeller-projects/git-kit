...
| `add-force-ignored` | `!f() { files=$(git ls-files --ignored --exclude-standard | sort -u); [ -z "$files" ] || git add --force -- $files; }; f` | WARNING: stages files that are explicitly gitignored — almost never intentional. Force-add gitignored files to the index. | dangerous |
...
| `unadd` | `restore --staged` | Unstage changes from the index (modern restore --staged, requires git >= 2.23) | medium |
